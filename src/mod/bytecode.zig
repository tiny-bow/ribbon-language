...
pub const TableBuilder = struct {
    ...
    pub fn bindBuiltinDataBuffer(self: *TableBuilder, id: core.BuiltinId, data: anytype) error{ BadEncoding, OutOfMemory }!void {
        // we must be tracking the id to manage the builtin data
        const entry = self.statics.getPtr(id.cast(anyopaque)) orelse {
            log.debug("TableBuilder.bindBuiltinDataBuffer: {} does not exist", .{id});
            return error.BadEncoding;
        };

        // type check: the entry must be a builtin
        if (entry.kind != .builtin) {
            log.debug("TableBuilder.bindBuiltinDataBuffer: expected builtin, got {s}", .{@tagName(entry.kind)});
            return error.BadEncoding;
        }
        ...
        // setup the content
        entry.content = .{ .builtin = .fromDataBuffer(data) };
    }
...
test "bind builtin data" {
    ...
    const builtin_id: core.BuiltinId = try tb.builder.createHeaderEntry(.builtin, "my_data");

    // Define some static data that will outlive the test
    const dummyBuiltinBuf: []const u8 = &.{ 0xDE, 0xAD, 0xBE, 0xEF };

    // Bind the native data to the ID
    try tb.builder.bindBuiltinDataBuffer(builtin_id, dummyBuiltinBuf);
