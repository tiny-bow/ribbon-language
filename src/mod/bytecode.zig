...
pub const TableBuilder = struct {
    ...
    /// Bind a static data buffer to an id.
    /// * a fresh id can be created with `createHeaderEntry`
    /// * The provided `data` must outlive any use of the generated bytecode, and is not managed by the runtime
    pub fn bindBuiltinData(self: *TableBuilder, id: core.BuiltinAddressId, data: core.BuiltinAddress.Buf) error{ BadEncoding, OutOfMemory }!void {
        // we must be tracking the id to manage the builtin data
        const entry = self.statics.getPtr(id.cast(anyopaque)) orelse {
            log.debug("TableBuilder.bindBuiltinData: {} does not exist", .{id});
            return error.BadEncoding;
        };

        // type check: the entry must be a builtin
        if (entry.kind != .builtin) {
            log.debug("TableBuilder.bindBuiltinData: expected builtin, got {s}", .{@tagName(entry.kind)});
            return error.BadEncoding;
        }
        ...
        // setup the content
        entry.content = .{ .builtin = .fromBuffer(data) };
    }

    /// Bind a `core.BuiltinFunction` to an id.
    /// * a fresh id can be created with `createHeaderEntry`
    /// * The provided `func` address must outlive any use of the generated bytecode
    pub fn bindBuiltinFunction(self: *TableBuilder, id: core.BuiltinAddressId, func: *const core.BuiltinFunction) error{ BadEncoding, OutOfMemory }!void {
        // we must be tracking the id to manage the builtin function
        const entry = self.statics.getPtr(id.cast(anyopaque)) orelse {
            log.debug("TableBuilder.bindBuiltinFunction: {} does not exist", .{id});
            return error.BadEncoding;
        };

        // type check: the entry must be a function
        if (entry.kind != .builtin) {
            log.debug("TableBuilder.bindBuiltinFunction: expected builtin, got {s}", .{@tagName(entry.kind)});
            return error.BadEncoding;
        }
        ...
        // setup the content
        entry.content = .{ .builtin = .fromFunction(func) };
    }
        ...
        while (static_it.next()) |pair| {
            ...
            if (entry.content) |builder| {
                switch (builder) {
                    ...
                    .builtin => |builtin| {
                        const loc = encoder.localLocationId(static_id.cast(core.BuiltinAddress));
...
fn dummyBuiltinFunction(fiber: *core.mem.FiberHeader) callconv(.C) core.BuiltinSignal {
    _ = fiber;
    return core.BuiltinSignal.@"return";
}
...
test "bind builtin function" {
    ...
    const builtin_id: core.BuiltinAddressId = try tb.builder.createHeaderEntry(.builtin, "my_builtin_func");

    // Bind the native Zig function to the ID
    try tb.builder.bindBuiltinFunction(builtin_id, dummyBuiltinFunction);
    ...
    const builtin_addr: *const core.BuiltinAddress = table.bytecode.header.get(builtin_id);
    try testing.expectEqual(builtin_addr.kind, .function);
    try testing.expectEqual(@intFromPtr(&dummyBuiltinFunction), @intFromPtr(builtin_addr.asFunction()));
...
test "bind builtin data" {
    var tb = try TestBed("bind builtin data").init();
    defer tb.deinit();

    // Create an entry for it in the header
    const builtin_id: core.BuiltinAddressId = try tb.builder.createHeaderEntry(.builtin, "my_data");

    // Define some static data that will outlive the test
    const dummyBuiltinBuf = core.BuiltinAddress.Buf.fromSlice(&.{ 0xDE, 0xAD, 0xBE, 0xEF });

    // Bind the native data to the ID
    try tb.builder.bindBuiltinData(builtin_id, dummyBuiltinBuf);

    // Create a main function to test referencing this data
    const main_id = try tb.builder.createHeaderEntry(.function, "main");
    var main_fn = try tb.builder.createFunctionBuilder(main_id);
    var entry_block = try main_fn.createBlock();

    // Get the address of the builtin, load a byte from it
    try entry_block.instrAddrOf(.addr_b, .r0, builtin_id);
    try entry_block.instr(.load8, .{ .Rx = .r1, .Ry = .r0, .I = 2 }); // Load 3rd byte (0xBE)
    try entry_block.instrTerm(.@"return", .{ .R = .r1 });
    ...
    // 1. Check the address in the header
    const builtin_addr: *const core.BuiltinAddress = table.bytecode.header.get(builtin_id);
    try testing.expectEqual(builtin_addr.kind, .data);
    try testing.expectEqual(dummyBuiltinBuf.ptr, builtin_addr.asBuffer().ptr);
    try testing.expectEqual(dummyBuiltinBuf.len, builtin_addr.asBuffer().len);
    ...
    const expected_disas =
        \\    addr_b r0 B:00000000
        \\    load8 r1 r0 I:00000002
        \\    return r1
        \\
    ;
    try testing.expectEqualStrings(expected_disas, disas_buf.items);
