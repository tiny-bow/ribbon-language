...
const Builder = @This();
pub const BlockBuilder = @import("RbcBuilder/BlockBuilder.zig");
pub const FunctionBuilder = @import("RbcBuilder/FunctionBuilder.zig");
pub const HandlerSetBuilder = @import("RbcBuilder/HandlerSetBuilder.zig");
...
pub const TypeMap = std.ArrayHashMapUnmanaged(Rbc.Info.Type, void, utils.SimpleHashContext, true);
pub const TypeList = std.ArrayListUnmanaged(Rbc.Info.TypeIndex);
...
pub const HandlerMap = EvidenceMap(Rbc.FunctionIndex);
pub const InstrList = std.ArrayListUnmanaged(Rbc.Instruction);

fn EvidenceMap(comptime T: type) type {
    return std.ArrayHashMapUnmanaged(Rbc.EvidenceIndex, T, utils.SimpleHashContext, false);
}


pub const Global = struct {
    index: Rbc.GlobalIndex,
    alignment: Rbc.Info.ValueAlignment,
    initial: []u8,
};
...
pub const Function = union(enum) {
    ...
    pub const Foreign = struct {
        parent: *Builder,
        evidence: ?Rbc.EvidenceIndex,
        index: Rbc.FunctionIndex,
        num_arguments: Rbc.RegisterIndex,
        num_registers: Rbc.RegisterIndex,

        pub fn assemble(self: *const Foreign) Rbc.Function.Foreign {
            return Rbc.Function.Foreign {
    ...
    pub fn assemble(self: Function, allocator: std.mem.Allocator) Error!union(enum) {bytecode: Rbc.Function, foreign: Rbc.Function.Foreign} {
        // TODO: the builder should be handling this
        var foreignId: Rbc.ForeignId = 0;
...
pub fn assemble(self: *const Builder, allocator: std.mem.Allocator) Error!Rbc.Program {
...
pub fn generateFunctionLists(self: *const Builder, allocator: std.mem.Allocator) Error!struct {[]Rbc.Function, []Rbc.Function.Foreign} {
    var functions = std.ArrayListUnmanaged(Rbc.Function){};
    var foreign_functions = std.ArrayListUnmanaged(Rbc.Function.Foreign){};
...
pub fn generateHandlerSetList(self: *const Builder, allocator: std.mem.Allocator) Error![]Rbc.Handler.Set {
    const handlerSets = try allocator.alloc(Rbc.Handler.Set, self.handler_sets.items.len);
...
pub fn getGlobal(self: *const Builder, index: Rbc.GlobalIndex) Error! *Global {
...
pub fn globalBytes(self: *Builder, alignment: Rbc.Info.ValueAlignment, initial: []u8) Error! *Global {
...
pub fn getFunction(self: *const Builder, index: Rbc.FunctionIndex) Error! *Function {
...
pub fn evidence(self: *Builder) Error! Rbc.EvidenceIndex {
...
pub fn getHandlerSet(self: *const Builder, index: Rbc.HandlerSetIndex) Error! *HandlerSetBuilder {
...
pub fn extractFunctionIndex(self: *const Builder, f: anytype) Error! Rbc.FunctionIndex {
    switch (@TypeOf(f)) {
        ...
        *const FunctionBuilder => {
            ...
            "invalid block index parameter, expected either `Rbc.FunctionIndex`, `*Builder.FunctionBuilder` or `Builder.Function`, got `{s}`",
            .{@typeName(@TypeOf(f))}
        )),
    }
}

pub fn extractHandlerSetIndex(self: *const Builder, h: anytype) Error! Rbc.HandlerSetIndex {
    switch (@TypeOf(h)) {
        ...
        *const HandlerSetBuilder => {
            ...
            "invalid handler set index parameter, expected either `Rbc.HandlerSetIndex` or `*Builder.HandlerSetBuilder`, got `{s}`",
            .{@typeName(@TypeOf(h))}
        )),
    }
}
