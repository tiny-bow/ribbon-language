...
pub fn @"display-width"(interpreter: *Rml.Interpreter, origin: Rml.Origin, args: []const Rml.Object) Rml.Result! Rml.Object {
    ...
    for (args) |arg| {
        if (Rml.castObj(Rml.String, arg)) |str| {
            a += @intCast(utils.text.displayWidthStr(str.data.text()) catch 0);
        } else if (Rml.castObj(Rml.Char, arg)) |char| {
...
fn textConv(comptime charConv: anytype, interpreter: *Rml.Interpreter, origin: Rml.Origin, args: []const Rml.Object) Rml.Result! Rml.Object {
    ...
    for (args) |arg| {
        if (Rml.castObj(Rml.String, arg)) |str| {
            ...
            var i: usize = 0;
            while (i < text.len) {
                const res = try utils.text.decode1(text[i..]);
                i += res.len;
    ...
    if (!compound) {
        if (utils.text.codepointCount(newStr.text()) catch 0 == 1) {
            return (try Rml.Obj(Rml.Char).wrap(Rml.getRml(interpreter), origin, utils.text.nthCodepoint(0, newStr.text()) catch unreachable orelse unreachable)).typeErase();
        }
    }
