...
pub const Case = union(enum) {
    ...
    pub fn parse(interpreter: *Rml.Interpreter, origin: Rml.Origin, args: []const Rml.Object) Rml.Result!Case {
        ...
        const case = if (Rml.object.isExactSymbol("else", args[0]) or Rml.object.isExactSymbol(CASE_SEP_SYM, args[0])) elseCase: {
            ...
            if (offset + 2 > args.len) try interpreter.abort(parseResult.value.getOrigin(), error.UnexpectedEOF, "expected {s} and a body to follow case scrutinizer pattern", .{Rml.object.procedure.CASE_SEP_SYM});

            const sepSym = Rml.castObj(Rml.Symbol, args[offset]) orelse {
                try interpreter.abort(args[offset].getOrigin(), error.UnexpectedInput, "expected {s} to follow case scrutinizer pattern, found {}", .{ Rml.object.procedure.CASE_SEP_SYM, args[offset] });
            };

            offset += 1;

            if (!std.mem.eql(u8, sepSym.data.text(), Rml.object.procedure.CASE_SEP_SYM)) {
                try interpreter.abort(sepSym.getOrigin(), error.UnexpectedInput, "expected {s} to follow case scrutinizer pattern, found {}", .{ Rml.object.procedure.CASE_SEP_SYM, sepSym });
            }
    ...
    pub fn call(self: *Procedure, interpreter: *Rml.Interpreter, callOrigin: Rml.Origin, blame: Rml.Object, args: []const Rml.Object) Rml.Result!Rml.Object {
        switch (self.*) {
            .cancellation => |cancellation| {
                ...
                interpreter.cancellation = .{ .with_id = cancellation, .output = if (eArgs.items.len == 0) (try Rml.Obj(Rml.Nil).wrap(Rml.getRml(interpreter), callOrigin, .{})).typeErase() else if (eArgs.items.len == 1) eArgs.items[0] else (try Rml.Obj(Rml.Array).wrap(Rml.getRml(interpreter), callOrigin, .{ .allocator = Rml.getRml(interpreter).blobAllocator(), .native_array = eArgs })).typeErase() };
                ...
                for (macro.cases.items) |case| switch (case) {
                    ...
                    .pattern => |caseData| {
                        ...
                        if (table) |tbl| {
                            ...
                            writer.print("failed to match; {} vs {any}:\n\t{}", .{ caseData.scrutinizer, args, d.formatter(error.PatternFailed) }) catch |err| return Rml.errorCast(err);
                        } else {
                            Rml.log.interpreter.err("requested pattern diagnostic is null", .{});
                            writer.print("failed to match; {} vs {any}", .{ caseData.scrutinizer, args }) catch |err| return Rml.errorCast(err);
                        }
                    },
                };
                ...
                for (func.cases.items) |case| switch (case) {
                    ...
                    .pattern => |caseData| {
                        ...
                        if (result) |res| {
                            ...
                            writer.print("failed to match; {} vs {any}:\n\t{}", .{ caseData.scrutinizer, eArgs, d.formatter(error.PatternFailed) }) catch |err| return Rml.errorCast(err);
                        } else {
                            Rml.log.interpreter.err("requested pattern diagnostic is null", .{});
                            writer.print("failed to match; {} vs {any}", .{ caseData.scrutinizer, eArgs }) catch |err| return Rml.errorCast(err);
                        }
                    },
                };
