...
body: common.ArrayList(bytecode.ProtoInstr) = .empty,
...
/// Append a pre-composed `bytecode.ProtoInstr` to the sequence.
...
pub fn proto(self: *SequenceBuilder, pi: bytecode.ProtoInstr) bytecode.Error!void {
...
pub fn instr(self: *SequenceBuilder, comptime code: bytecode.Instruction.BasicOpCode, data: bytecode.Instruction.OperandSet(code.upcast())) bytecode.Error!void {
    try self.proto(.{
        .instruction = .{
            .code = code.upcast(),
...
pub fn instrWide(self: *SequenceBuilder, comptime code: bytecode.Instruction.WideOpCode, data: bytecode.Instruction.OperandSet(code.upcast()), wide_operand: bytecode.Instruction.WideOperand(code.upcast())) bytecode.Error!void {
    try self.proto(.{
        .instruction = .{
            .code = code.upcast(),
            .data = @unionInit(bytecode.Instruction.OpData, @tagName(code), data),
...
pub fn AddrId(comptime code: bytecode.Instruction.AddrOpCode) type {
...
pub fn instrAddrOf(self: *SequenceBuilder, comptime code: bytecode.Instruction.AddrOpCode, register: core.Register, id: AddrId(code)) binary.Encoder.Error!void {
    const Set = comptime bytecode.Instruction.OperandSet(code.upcast());
    ...
    try self.proto(.{
        .instruction = .{
            .code = code.upcast(),
            .data = @unionInit(bytecode.Instruction.OpData, @tagName(code), set),
        },
        .additional = .none,
    });
}
...
pub fn instrCall(self: *SequenceBuilder, comptime code: bytecode.Instruction.CallOpCode, data: bytecode.Instruction.OperandSet(code.upcast()), args: common.Buffer.fixed(core.Register, core.MAX_REGISTERS)) bytecode.Error!void {
    try self.proto(.{
        .instruction = .{
            .code = code.upcast(),
            .data = @unionInit(bytecode.Instruction.OpData, @tagName(code), data),
...
pub fn pushHandlerSet(self: *SequenceBuilder, handler_set: *bytecode.HandlerSetBuilder) binary.Encoder.Error!void {
...
pub fn popHandlerSet(self: *SequenceBuilder, handler_set: *bytecode.HandlerSetBuilder) binary.Encoder.Error!void {
    if (handler_set.function != self.function) {
...
pub fn bindHandlerSetCancellationLocation(self: *SequenceBuilder, handler_set: *bytecode.HandlerSetBuilder) binary.Encoder.Error!void {
    if (handler_set.function != self.function) {
...
pub fn encode(self: *const SequenceBuilder, upvalue_fixups: ?*const bytecode.UpvalueFixupMap, local_fixups: *const bytecode.LocalFixupMap, encoder: *binary.Encoder) binary.Encoder.Error!void {
