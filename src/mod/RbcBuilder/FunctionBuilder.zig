...
pub fn init(parent: *Builder, index: Rbc.FunctionIndex) Error!*FunctionBuilder {
...
pub fn assemble(self: *const FunctionBuilder, allocator: std.mem.Allocator) Error!Rbc.Function {
...
pub fn getBlock(self: *const FunctionBuilder, index: Rbc.BlockIndex) Error!*BlockBuilder {
...
pub fn newBlock(self: *FunctionBuilder, parent: ?Rbc.BlockIndex, kind: BlockBuilder.Kind) Error!*BlockBuilder {
    const index = self.blocks.items.len;
    if (index >= Rbc.MAX_BLOCKS) return Error.TooManyBlocks;
...
pub fn arg(self: *FunctionBuilder) Error!Rbc.RegisterIndex {
...
pub fn local(self: *FunctionBuilder) Error!Rbc.RegisterIndex {
...
pub fn upvalue(self: *FunctionBuilder) Error!Rbc.UpvalueIndex {
