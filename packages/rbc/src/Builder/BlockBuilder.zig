...
pub fn op(self: *BlockBuilder, code: Core.Op.Code, data: *const anyopaque) Error!void {
    if (self.exited) return Error.InstructionsAfterExit;

    try self.ops.append(self.function.parent.allocator, Core.Instruction {
        .code = code,
        .data = inline for (comptime std.meta.fieldNames(Core.Op.Code)) |name| {
            if (@field(Core.Op.Code, name) == code) {
                const T = @TypeOf(@field(@as(Core.Op.Data, undefined), name));
                break @unionInit(Core.Op.Data, name, if (T == void) {} else @as(*const T, @ptrCast(@alignCast(data))).*);
            }
        } else unreachable,
    });
}

pub fn exitOp(self: *BlockBuilder, comptime code: Core.Op.Code, data: *const anyopaque) Error!void {
